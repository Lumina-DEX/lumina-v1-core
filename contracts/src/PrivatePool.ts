import { Field, SmartContract, Permissions, state, State, method, TokenContractV2, PublicKey, AccountUpdateForest, DeployArgs, UInt64, AccountUpdate, Provable, VerificationKey, TokenId, Account, Bool, Int64, Reducer, Struct, CircuitString, ZkProgram, PrivateKey, Signature } from 'o1js';
...
export class Private extends TokenContractV2 {
    ...
    @method async swapFromToken(amountTokenIn: UInt64, amountMinaOutMin: UInt64, balanceInMax: UInt64, balanceOutMin: UInt64, proof: MainProof) {
        ...
        proof.verify();
        let sender = this.sender.getUnconstrained();
        proof.publicInput.equals(sender).assertTrue("Proof doesn't match sender");
