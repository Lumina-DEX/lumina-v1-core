 ...
 */
export class TokenAdminMint extends SmartContract implements FungibleTokenAdminBase {
    @state(PublicKey)
    private adminPublicKey = State<PublicKey>()

    @state(Bool)
    alreadyMint = State<Bool>();
    ...
    public async canMint(_accountUpdate: AccountUpdate) {
        const alreadyMint = this.alreadyMint.getAndRequireEquals();
        alreadyMint.assertFalse("You can mint only once");
        await this.ensureAdminSignature()
        this.alreadyMint.set(Bool(true));
        return Bool(true)
    }
