...
import { AccountUpdate, Bool, fetchAccount, Field, Mina, PrivateKey, PublicKey, UInt64, UInt8, VerificationKey } from 'o1js';
...
describe('Pool data', () => {
  ...
  it('update pool verification key', async () => {
    ...
    const newVk = new VerificationKey({ data: dataPool, hash: hashPool });
    let txn = await Mina.transaction(senderAccount, async () => {
      await zkPoolData.setPoolVerificationKey(newVk);
    });
    await txn.prove();
    await txn.sign([senderKey, bobKey]).send();
  ...
  it('update verification key', async () => {
    ...
    const newVk = new VerificationKey({ data: dataPool, hash: hashPool });
    let txn = await Mina.transaction(senderAccount, async () => {
      await zkPoolData.updateVerificationKey(newVk);
    });
    await txn.prove();
    await txn.sign([senderKey, bobKey]).send();
