...
import { Faucet } from '../indexmina';
import { TokenAdminMint } from '../TokenAdminMint';
import { FungibleTokenMint } from '../FungibleTokenMint';
...
describe('Token', () => {
  let deployerAccount: Mina.TestPublicKey,
    ...
    zkTokenAdmin: TokenAdminMint,
    zkTokenAddress: PublicKey,
    zkTokenPrivateKey: PrivateKey,
    zkToken: FungibleTokenMint,
  ...
  beforeAll(async () => {
    ...
    if (proofsEnabled) {
      ...
      await TokenAdminMint.compile();
      await FungibleTokenMint.compile();
  ...
  beforeEach(async () => {
    ...
    zkTokenAdmin = new TokenAdminMint(zkTokenAdminAddress);

    zkTokenPrivateKey = PrivateKey.random();
    zkTokenAddress = zkTokenPrivateKey.toPublicKey();
    zkToken = new FungibleTokenMint(zkTokenAddress);
  ...
  it('mint once', async () => {
    ...
    txn = await Mina.transaction(bobAccount, async () => {
      await expect(zkToken.mint(zkFaucetAddress, amt)).rejects.toThrow();
    });
    // console.log("mint", txn.toPretty());
    // await txn.prove();
    // await expect(txn.sign([bobKey]).send()).rejects.toThrow();
