...
import { FungibleToken, FungibleTokenAdmin, mulDiv, Faucet, PoolFactory, PoolTokenHolder, Pool, SignerMerkleWitness } from '../index.js';
    ...
    try {
        ...
        let tx = await Mina.transaction(
            ...
            async () => {
                ...
                await zkFactory.createPoolToken(zkAppAddress, zkTokaAddress, zkTokenAddress, zkTokenAddress, signature, circuitWitness);
            }
        );
        await tx.prove();
...
async function swapMina() {
    try {
        ...
        let tx = await Mina.transaction({ sender: feepayerAddress, fee }, async () => {
            await dexTokenHolder.swapFromTokenToToken(feepayerAddress, UInt64.from(5), amountIn, UInt64.from(1), balanceMax, balanceMin);
...
async function swapToken() {
    try {
        ...
        let tx = await Mina.transaction({ sender: feepayerAddress, fee }, async () => {
            await zkApp.swapFromTokenToMina(feepayerAddress, UInt64.from(5), amountIn, expectedOut, balanceMax, balanceMin);
        });
        await tx.prove();
