...
import { Pool, TokenHolder, TokenStandard } from '../index.js';

// check command line arg
let deployAlias = "newpool";
if (!deployAlias)
    throw Error(`Missing <deployAlias> argument.

Usage:
node build/src/deploy/poolholder.js
`);
...
let zkAppKeysBase58: { privateKey: string; publicKey: string } = JSON.parse(
    await fs.readFile("keys/newpool.json", 'utf8'));
...
let zkToken1 = new TokenStandard(zkToken1Address);
